...
describe('selectCandidates', () => {
  ...
  it('adds a dated book not already known, using its dateRead year', () => {
    ...
    const result = selectCandidates(dated, [], new Set(), new Set(), new Set(), 14, now);
  ...
  it('skips a dated book whose exact bookId+dateRead is already known', () => {
    const dated = [book({ bookId: 'a', dateRead: 'Tue, 18 Aug 2020 00:00:00 +0000' })];
    const result = selectCandidates(dated, [], new Set(['a']), new Set(['a|2020-08-18']), new Set(['a']), 14, now);
    expect(result).toHaveLength(0);
  });

  it('adds a re-read: same bookId, a new dateRead, and prior dated evidence for that bookId', () => {
    const dated = [book({ bookId: 'a', dateRead: 'Mon, 24 Aug 2026 00:00:00 +0000' })];
    const result = selectCandidates(
      dated,
      [],
      new Set(['a']),
      new Set(['a|2020-08-18']),
      new Set(['a']),
      14,
      now
    );
  ...
  it('does not treat a newly-dated, previously-undated book as a re-read', () => {
    // Same read event: the book was filed undated before, and Goodreads now
    // reports a dateRead for it. There's no prior *dated* entry for this
    // bookId, so this isn't a re-read — it should be skipped, not duplicated.
    const dated = [book({ bookId: 'a', dateRead: 'Mon, 24 Aug 2026 00:00:00 +0000' })];
    const result = selectCandidates(dated, [], new Set(['a']), new Set(), new Set(), 14, now);
    expect(result).toHaveLength(0);
  });
  ...
  it('adds an undated book only if added within the lookback window, filed under the undated bucket', () => {
    ...
    const result = selectCandidates([], [withinWindow, outsideWindow], new Set(), new Set(), new Set(), 14, now);
  ...
  it('drops an undated book with no dateAdded at all', () => {
    ...
    const result = selectCandidates([], [noDateAdded], new Set(), new Set(), new Set(), 14, now);
    expect(result).toHaveLength(0);
  });
  ...
  it('does not double-add a book present in both shelves', () => {
    ...
    const result = selectCandidates(dated, defaultOrder, new Set(), new Set(), new Set(), 14, now);
    expect(result).toHaveLength(1);
  ...
  it('skips an already-known book in the undated pass too', () => {
    ...
    const result = selectCandidates([], [undated], new Set(['f']), new Set(), new Set(), 14, now);
    expect(result).toHaveLength(0);
  });
});
