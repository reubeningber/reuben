...
src/
  ...
  components/
    ...
    ReadingCover.astro  # Book cover card for /reading — hover-reveals a genre badge (top-left) and a 5-star rating row (bottom); data-title/data-author/data-audio/data-rating/data-genre attrs drive client-side filtering
  pages/
  ...
  utils/
    icons.ts             # Shared inline-SVG star path constants (STAR_FILLED/STAR_EMPTY), used by ReadingCover.astro and reading.astro
  styles/
    global.css
```
...
- The accent color is orange (`text-accent` = `#c2410c`, i.e. orange-700 — darkened from orange-600 to pass WCAG AA contrast on white); primary text is dark (`text-primary`)
...
| `ANTHROPIC_API_KEY` | Optional. Used by `scripts/lib/genre.mjs` to classify new books' genre; falls back to a weaker Open Library keyword heuristic when unset. Not used by the site build |
