...
src/
  ...
  components/
    ...
    ReadingCover.astro  # Book cover card for /reading — hover-reveals a 5-star rating row; data-title/data-author/data-audio/data-rating attrs drive client-side filtering
  pages/
    ...
    reading.astro                       # Tabbed year-by-year book log (2020-2026) — covers link out to Amazon; search by title/author (spans all years); a "Filters" popover (icon-only on mobile) holds an audiobooks toggle and an exact-star-rating picker, both of which filter within the currently selected year
    rss.xml.js                          # RSS feed
